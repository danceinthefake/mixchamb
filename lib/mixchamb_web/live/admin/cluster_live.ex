defmodule MixchambWeb.Admin.ClusterLive do
  @moduledoc """
  ...
  Two operations beyond observation:
    ...
    * **Drain**: kill the target node's `MixchambWeb.Endpoint`. The
      ...
      not String.contains?(target, "@") ->
        {:noreply,
         assign(
           socket,
           :connect_error,
           "Node name needs an @ — e.g. 'mixchamb2@hostname'."
         )}
  ...
  def handle_event("disconnect", %{"node" => target}, socket) do
    ...
    Mixchamb.Audit.log_as(socket.assigns.current_admin, "disconnect_node", "node:#{node}", %{})
  ...
  def handle_event("drain", %{"node" => target}, socket) do
    ...
    Mixchamb.Audit.log_as(socket.assigns.current_admin, "drain_node", "node:#{node}", %{})
  ...
  @doc """
  Cleanly cycles the local node's `MixchambWeb.Endpoint`: ask the
  ...
  """
  def drain_local do
    case Supervisor.terminate_child(Mixchamb.Supervisor, MixchambWeb.Endpoint) do
      :ok ->
        case Supervisor.restart_child(Mixchamb.Supervisor, MixchambWeb.Endpoint) do
          {:ok, _pid} -> :ok
          {:ok, _pid, _info} -> :ok
  ...
  def render(assigns) do
    ...
    >
      ...
      >
        <div class="flex-1 min-w-[16rem] space-y-1">
          ...
          <input
            ...
            phx-debounce="200"
            placeholder="e.g. mixchamb2@your-hostname"
      ...
      <div class="mt-8 rounded-xl border bg-card/50 p-4 space-y-2 text-xs text-muted-foreground">
        ...
        <pre class="whitespace-pre font-mono text-[11px] bg-muted/40 rounded p-3 mt-1 overflow-x-auto"><%= "Terminal 1: PORT=4000 iex --sname mixchamb1 --cookie shared -S mix phx.server\nTerminal 2: PORT=4001 SKIP_VITE=1 iex --sname mixchamb2 --cookie shared -S mix phx.server" %></pre>
        <p class="text-[11px]">
        ...
        <p>
          ...
          on the remote row kills its <code class="px-1 rounded bg-muted">MixchambWeb.Endpoint</code>
          via :rpc;
