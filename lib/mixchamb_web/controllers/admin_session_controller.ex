defmodule MixchambWeb.AdminSessionController do
  @moduledoc """
  Login + logout for the `/admin` section.

  Authentication tries two sources, in order:

    1. **Database** — `Mixchamb.Admins.authenticate/2`. Per-user
  ...
  defp verify(user, pass) when is_binary(user) and is_binary(pass) do
    case Admins.authenticate(user, pass) do
      %Mixchamb.Admins.Admin{username: username} ->
        {:ok, username}

      nil ->
        if env_match?(user, pass) do
          {:ok, Application.get_env(:mixchamb, :admin_user, "admin")}
        else
          :error
        end
    end
  ...
  defp env_match?(user, pass) do
    expected_user = Application.get_env(:mixchamb, :admin_user)
    expected_pass = Application.get_env(:mixchamb, :admin_password)
