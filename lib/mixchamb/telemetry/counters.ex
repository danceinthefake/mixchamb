defmodule Mixchamb.Telemetry.Counters do
  @moduledoc """
  Subscribes to mixchamb's custom telemetry events and maintains
  ...
  def handle_event([:mixchamb, :chamber, :note], _measurements, metadata, _config) do
    GenServer.cast(__MODULE__, {:note, metadata})
  end

  def handle_event([:mixchamb, :chamber, :note_dropped], _, _, _) do
    GenServer.cast(__MODULE__, :note_dropped)
  end

  def handle_event([:mixchamb, :chamber, :created], _, _, _) do
    GenServer.cast(__MODULE__, :created)
  end

  def handle_event([:mixchamb, :chamber, :deleted], _, _, _) do
    GenServer.cast(__MODULE__, :deleted)
  end

  def handle_event([:mixchamb, :chamber, :restarted], _, _, _) do
