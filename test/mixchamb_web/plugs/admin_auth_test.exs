defmodule MixchambWeb.Plugs.AdminAuthTest do
  use MixchambWeb.ConnCase, async: false

  alias MixchambWeb.Plugs.AdminAuth
  ...
  describe "fails closed when no admin password is configured" do
    setup do
      original = Application.get_env(:mixchamb, :admin_password)

      on_exit(fn ->
        Application.put_env(:mixchamb, :admin_password, original)
      end)

      Application.put_env(:mixchamb, :admin_password, nil)
      :ok
    end
