defmodule Mixchamb.Telemetry.CountersTest do
  use ExUnit.Case, async: false

  alias Mixchamb.Telemetry.Counters
  ...
  describe "note events bump counters + per-instrument map" do
    test "executing the [:mixchamb, :chamber, :note] event increments the counters" do
  ...
  describe "lifecycle events" do
    test "created / deleted / restarted each bump their own counter" do
      ...
      :telemetry.execute([:mixchamb, :chamber, :created], %{count: 1}, %{slug: "a"})
      :telemetry.execute([:mixchamb, :chamber, :deleted], %{count: 1}, %{slug: "a"})
      :telemetry.execute([:mixchamb, :chamber, :restarted], %{count: 1}, %{slug: "a"})
    ...
    test "note_dropped bumps total_notes_dropped" do
      ...
      :telemetry.execute([:mixchamb, :chamber, :note_dropped], %{count: 1}, %{slug: "a"})
